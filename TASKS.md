...
| T0.3 | DONE | CLI entry point with `click`: `sphereforge process`, `sphereforge export`, `sphereforge status` | T0.1, T0.2 | GLUE | — | — | `process` runs stages 1-8 with `--stage`, `--resume`, `--config`. `export` runs Stage 8. Fixed 2026-05-02. |
...
| T0.10 | DONE | Dockerfile / conda environment: CUDA 12.8+, PyTorch, COLMAP CLI, Python deps | T0.1 | GLUE | — | — | `Dockerfile` + `environment.yml`. Fixed 2026-05-03. |
...
| T0.12 | DONE | Integration test harness: `tests/integration/test_pipeline.py` — runs stages 2→5 on synthetic data | T0.11 | TEST | — | — | Validates full pipeline on tiny synthetic scene. Fixed 2026-05-03. |
...
| T2.11 | DONE | Stage 2 orchestration: `run_stage02()` — connect T2.1→T2.10 into a single pipeline function, writes COLMAP dataset + RPG360 depth to `data/cubemaps/` | T2.1–T2.10 | GLUE | — | — | Config selects RPG360 depth on/off. Parallel processing via `ProcessPoolExecutor` when `num_workers > 1`. Fixed duplicate crop resize. Fixed 2026-05-02. |
| T2.12 | DONE | Stage 2 unit tests: test cubemap extraction geometry, yaw offset logic, intrinsics/extrinsics values, YOLO mask generation, RPG360 graph optimization on synthetic depth | T2.11 | TEST | — | — | `tests/test_stage02.py` created with 14 tests covering cubemap, intrinsics, extrinsics, yaw diversify, masking, COLMAP writers. Fixed 2026-05-02. |
| T2.13 | DONE | Panorama-rig virtual camera layout: render fixed rectilinear views from ERP panoramas into per-camera folders and persist a rig manifest for Stage 3 | T2.11 | GLUE | COLMAP panorama_sfm.py | BSD-2 | Added `panorama_rig` Stage 2 path with 4x3 virtual cameras, stable folder naming, and `panorama_rig.json` metadata. Implemented 2026-05-15. |
...
| T3.8 | DONE | Panorama-rig aware Stage 3 ingestion: recursively discover rendered views, use per-folder cameras, preserve rig-friendly ordering, and adapt matcher selection for panorama-rig datasets | T3.1, T3.2, T3.6, T2.13 | ADAPT | COLMAP panorama_sfm.py | BSD-2 | Uses per-folder image ordering and `single_camera_per_folder` when `panorama_rig.json` is present. Implemented 2026-05-15. |
| T3.9 | DONE | Panorama-rig regression tests and COLMAP CLI compatibility fallback for feature extraction | T3.7, T3.8 | TEST | — | — | Added panorama-rig tests plus RootSIFT fallback to older COLMAP builds that only support `--descriptor_normalization`. Implemented 2026-05-15. |
...
| T4.6 | DONE | Stage 4 orchestration: `run_stage04()` — connect T4.1→T4.5, writes metric depth maps to `data/depth/` | T4.1–T4.5 | GLUE | — | — | Logs per-frame depth stats. Fail-fast on `NotImplementedError` instead of silent zero-depth fallback. Fixed 2026-05-02. |
...
| T5.12 | DONE | Stage 5 orchestration: `run_stage05()` — connect T5.1→T5.11, writes initial .ply to `data/gaussians/` | T5.1–T5.11 | GLUE | — | — | Logs Gaussian count and stats. Now uses per-pixel `stride_map` from `assign_stride()` instead of uniform stride. Fixed 2026-05-02. |
...
| T6.11 | DONE | Training loop: `train_gaussians()` — main optimization loop with configurable iterations, densification schedule (EAS every 500 iters), RAP pruning schedule (every 3000 iters), SH expansion (degree 0→3 at 7500 iters) | T6.1–T6.10 | ADAPT | LichtFeld-Studio + ImprovedGS+ | GPLv3 / NC-3DGS | Raises `RuntimeError` with install instructions when gsplat is missing. Saves intermediate `.ply` checkpoints every `checkpoint_every` iterations (default 5000). Fixed 2026-05-02. |
...
| T7.3 | DONE | Gap classification: `classify_gaps()` — categorize holes as behind-foreground / at-boundary / missing-geometry using depth discontinuity analysis | T7.2 | REUSE | SPAG-4D | MIT | Now integrated into `fill_holes_iterative` (2026-05-14). Guides budget allocation and fill strategy. |
...
| T7.6 | DONE | ShareGS quick optimization: `blend_fill()` — lightweight k-NN smoothing to blend new Gaussians with surrounding scene | T7.4, T7.5 | GLUE | — | — | Original design used differentiable rendering; implemented as geometry-driven k-NN smoothing instead (no rasterizer dependency). Wires into `fill_holes_iterative` via `sharegs_optimize_iters` config. Added 2026-05-14. |
...
| T7.11 | DONE | Iterative hole-filling loop: `fill_holes_iterative()` — repeat hole detection → gap classification → weighted-budget ShareGS fill → blend → GS-Diff fill until hole coverage < 2% or max 2 rounds | T7.2, T7.6, T7.10 | GLUE | — | — | Checks convergence after each round. Added gap-classification-aware budget allocation and source-view filtering 2026-05-14. |
