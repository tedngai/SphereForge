...
def read_colmap_cameras(path: Path) -> dict[int, dict]:
    """Parse a COLMAP cameras.txt file.

    The COLMAP cameras.txt format has one camera per line (excluding comments
    and the count header) with the schema::
    ...
    Returns:
        Dictionary mapping ``camera_id`` (int) to a dict with keys:
        ``model`` (str), ``width`` (int), ``height`` (int),
        ``params`` (list[float]).

    Raises:
        FileNotFoundError: If the file does not exist.
        ValueError: If a camera line cannot be parsed.
    """
    path = Path(path)
    if not path.exists():
    ...
    with open(path) as f:
        for line_no, raw_line in enumerate(f, start=1):
            ...
            if len(parts) < 4:
                logger.warning(
                    "Skipping malformed line %d in %s: %s", line_no, path, line
                )
                continue
            try:
                ...
                raise ValueError(
                    f"Failed to parse camera line {line_no} in {path}: {line!r}"
                ) from exc
            cameras[camera_id] = {
                "model": model,
                "width": width,
    ...
    logger.info("Read %d cameras from %s", len(cameras), path)
    return cameras


def read_colmap_images(path: Path) -> dict[int, dict]:
    """Parse a COLMAP images.txt file.

    The COLMAP images.txt format uses two lines per image. The first line
    contains metadata::

        IMAGE_ID  QW  QX  QY  QZ  TX  TY  TZ  CAMERA_ID  NAME

    The second line lists 2-D point observations as repeated triplets::

        X  Y  POINT3D_ID

    Only the point3D_ids are extracted from the second line.

    Args:
        path: Path to the images.txt file.

    Returns:
        Dictionary mapping ``image_id`` (int) to a dict with keys:
        ``name`` (str), ``qw`` (float), ``qx`` (float), ``qy`` (float),
        ``qz`` (float), ``tx`` (float), ``ty`` (float), ``tz`` (float),
        ``camera_id`` (int), ``point3D_ids`` (list[int]).

    Raises:
        FileNotFoundError: If the file does not exist.
        ValueError: If an image line cannot be parsed.
    """
    path = Path(path)
    if not path.exists():
    ...
    # Lines come in pairs: metadata then point observations
    i = 0
    while i < len(lines):
        meta_parts = lines[i].split()
        if len(meta_parts) < 10:
            logger.warning(
                "Skipping malformed image metadata line %d in %s: %s",
                i + 1,
                path,
                lines[i],
            )
        ...
        try:
            ...
            qw = float(meta_parts[1])
            qx = float(meta_parts[2])
            qy = float(meta_parts[3])
            qz = float(meta_parts[4])
            tx = float(meta_parts[5])
            ty = float(meta_parts[6])
            tz = float(meta_parts[7])
            camera_id = int(meta_parts[8])
            name = meta_parts[9]
        except (ValueError, IndexError) as exc:
            raise ValueError(
                f"Failed to parse image metadata line {i + 1} in {path}: "
                f"{lines[i]!r}"
            ) from exc

        # Parse point3D_ids from the next line
        point3D_ids: list[int] = []
        if i + 1 < len(lines):
            pts_parts = lines[i + 1].split()
            # Observations come in triplets: (x, y, point3D_id)
            if len(pts_parts) % 3 != 0:
                logger.warning(
                    "Point observations line %d in %s has %d values "
                    "(expected a multiple of 3); truncating.",
                    i + 2,
                    path,
                    len(pts_parts),
                )
            for j in range(0, len(pts_parts) - 2, 3):
                try:
                    ...
                    logger.warning(
                        "Skipping invalid point3D_id at position %d on line %d in %s",
                        j + 2,
                        i + 2,
                        path,
                    )
    ...
    logger.info("Read %d images from %s", len(images), path)
    return images
